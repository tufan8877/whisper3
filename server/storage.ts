...
export class DatabaseStorage implements IStorage {
  ...
  /**
   * ✅ WICHTIG: Filtert Chats raus, die für diesen User gelöscht wurden (deletedChats Tabelle)
   */
  ...
  async incrementUnreadCount(chatId: number, userId: number): Promise<void> {
    ...
    if (chat.participant1Id === userId) {
      await db.update(chats).set({ unreadCount1: sql`${chats.unreadCount1} + 1` }).where(eq(chats.id, chatId));
    } else if (chat.participant2Id === userId) {
      await db.update(chats).set({ unreadCount2: sql`${chats.unreadCount2} + 1` }).where(eq(chats.id, chatId));
    }
  }
  ...
  /**
   * ✅ Persistent contacts: gleiche Filter-Logik wie Chats (deletedChats raus)
   */
...
export class MemStorage implements IStorage {
  ...
  private async loadPersistedData() {
    ...
    if (chatsData) {
      Object.entries(chatsData.chats || {}).forEach(([id, chat]: any) => {
        // revive createdAt + lastMessageTimestamp
        if (chat?.createdAt) chat.createdAt = this.reviveDate(chat.createdAt) || new Date();
        if (chat?.lastMessageTimestamp) chat.lastMessageTimestamp = this.reviveDate(chat.lastMessageTimestamp) || new Date();
  ...
  async incrementUnreadCount(chatId: number, userId: number): Promise<void> {
    ...
    if (chat.participant1Id === userId) (chat as any).unreadCount1 = ((chat as any).unreadCount1 ?? 0) + 1;
    if (chat.participant2Id === userId) (chat as any).unreadCount2 = ((chat as any).unreadCount2 ?? 0) + 1;
  ...
  async getPersistentChatContacts(
    ...
    // identisch wie getChatsByUserId – aber als “contacts”
...
/**
 * ✅ Wähle hier aus:
 * - Wenn du Postgres/Render DB nutzt: new DatabaseStorage()
 * - Wenn du Mem/Files nutzt: new MemStorage()
 *
 * Du hattest MemStorageEnhanced — die ist jetzt nicht mehr nötig.
 */
